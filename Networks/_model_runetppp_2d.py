...
from Networks.arch_parts1 import *
...
def runetppp_2d_base(input_tensor, filter_n_down, filter_n_skip, filter_n_combined,
                    ...
                    weights='imagenet', freeze_batch_norm=True, net_name='runetppp'):
    ...
    input_ten = convolutional_stack(input_ten, filter_n_down[0], kernel_size=3, stack_count=stack_count_down,
                                    activation=activation, apply_batch_norm=apply_batch_norm, block_name='{}_down0'.format(net_name))
    ...
    for i, filters in enumerate(filter_n_down[1:]):
        ...
        input_ten = unet_encoder_block(input_ten, filters, kernel_size=3, stack_count=stack_count_down, activation=activation,
                                       pooling=pool, apply_batch_norm=apply_batch_norm, block_name='{}_down{}'.format(net_name, i + 1))
        ...
        if i < len(filter_n_down) - 2:  # Exclude the last downsampled tensor
            ...
            input_ten = add([input_ten, input_ten_residual], name='{}_encoder_residual{}'.format(net_name, i + 1))
    ...
    for i in range(depth_decode):
        ...
        for level in range(depth_decode):
            ...
            if pool_scale < 0:
                ...
                input_ten = decode_block(decoder_tensors[level], filters, pool_size, unpool,
                                         activation=activation, apply_batch_norm=apply_batch_norm,
                                         block_name='{}_up_{}_en{}'.format(net_name, i, level))
            ...
            # Shallower tensors are obtained from **encoder** outputs
                ...
                input_ten = encode_block(encoder_tensors[level], filters, pool_size, pool, activation=activation,
                                         apply_batch_norm=apply_batch_norm, block_name='{}_down_{}_en{}'.format(net_name, i, level))

            # Convolutional layer after feature map scale change
            input_ten = convolutional_stack(input_ten, filters, kernel_size=3, stack_count=1,
                                            activation=activation, apply_batch_norm=apply_batch_norm,
                                            block_name='{}_down_from{}_to{}'.format(net_name, i, level))
        ...
        concatenated_tensor = concatenate(fusion_tensors, axis=-1, name='{}_concat_{}'.format(net_name, i))

        # Apply convolutional stack to the concatenated tensor
        input_ten = convolutional_stack(concatenated_tensor, filter_n_combined, kernel_size=3, stack_count=stack_count_up,
                                        activation=activation, apply_batch_norm=True,
                                        block_name='{}_fusion_conv_{}'.format(net_name, i))
    ...
    if depth_decode < depth - 1:
        for i in range(depth - depth_decode - 1):
            ...
            input_ten = unet_decoder_block(input_ten, None, filter_n_combined, stack_count=stack_count_up, activation=activation,
                                           upsampling=unpool, apply_batch_norm=apply_batch_norm, concatenate=False,
                                           block_name='{}_plain_up{}'.format(net_name, i_real))
            decoder_tensors.append(input_ten)
...
def runetppp_2d(input_size, n_class, filter_n_down=[64, 128, 256, 512], filter_n_skip='default', filter_n_combined='default',
               ...
               apply_batch_norm=False, pool='max', unpool=False, deep_supervision=False,
               weights='imagenet', freeze_batch_norm=True, net_name='runetpp'):
    ...
    decoder_tensors = runetppp_2d_base(input_tensor, filter_n_down, filter_n_skip, filter_n_combined,
                                      ...
                                      weights=weights, freeze_batch_norm=freeze_batch_norm, net_name=net_name)
    decoder_tensors = decoder_tensors[::-1]

    if deep_supervision:

        output_stack = []
        output_length = len(decoder_tensors)
        ...
        for i in range(1, output_length):
            ...
            input_ten = Conv2D(n_class, 3, padding='same', name='{}_output_conv_{}'.format(net_name, i - 1))(decoder_tensors[i])

            input_ten = decode_block(input_ten, n_class, pool_size, unpool,
                                     activation=None, apply_batch_norm=False, block_name='{}_output_sup{}'.format(net_name, i - 1))

            if output_activation:
                print('\t{}_output_sup{}_activation'.format(net_name, i - 1))
                input_ten = Activation('sigmoid', name='{}_output_sup{}_activation'.format(net_name, i - 1))(input_ten)

            else:
                if unpool is False:
                    print('\t{}_output_sup{}_trans_conv'.format(net_name, i - 1))
                else:
                    print('\t{}_output_sup{}_unpool'.format(net_name, i - 1))

            output_stack.append(input_ten)

        input_ten = convolutional_output(decoder_tensors[0], n_class, kernel_size=3,
                                         activation=output_activation, block_name='{}_output_final'.format(net_name))
        output_stack.append(input_ten)

        if output_activation:
            print('\t{}_output_final_activation'.format(net_name))
        else:
            print('\t{}_output_final'.format(net_name))

        model = Model([input_tensor, ], output_stack)
