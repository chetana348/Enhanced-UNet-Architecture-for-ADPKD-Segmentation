...
from Networks import SwitchNormalization
from tensorflow.keras.layers import ReLU, LeakyReLU, PReLU, ELU, Softmax

def decode_layer(X, channel, pool_size, unpool, kernel_size=3, 
                 activation='ReLU', switch_norm=False, name='decode'):
    '''
    ...
    * The defaut: `kernel_size=3`, is suitable for `pool_size=2`.
    ...
    if unpool:
        ...
        # batch normalization
        if switch_norm:
            X = SwitchNormalization(axis=3, name='{}_bn'.format(name))(X)
...
def encode_layer(X, channel, pool_size, pool, kernel_size='auto', 
                 activation='ReLU', switch_norm=False, name='encode'):
    '''
...
def CONV_stack(X, channel, kernel_size=3, stack_num=2, 
               ...
               switch_norm=False, name='conv_stack'):
    '''
    Stacked convolutional layers:
    (Convolutional layer --> switch normalization --> Activation)*stack_num
    ...
    for i in range(stack_num):
        ...
        # switch normalization
        if switch_norm:
            X = SwitchNormalization(axis=3, name='{}_{}_bn'.format(name, i))(X)
...
def Res_CONV_stack(X, X_skip, channel, res_num, activation='ReLU', switch_norm=False, name='res_conv'):
    '''
    Stacked convolutional layers with residual path.
     
    Res_CONV_stack(X, X_skip, channel, res_num, activation='ReLU', switch_norm=False, name='res_conv')
...
def Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, dilation_rate=1, activation='ReLU', switch_norm=False, name='sep_conv'):
    '''
    Depthwise separable convolution with (optional) dilated convolution kernel and switch normalization.
    
    Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, dilation_rate=1, activation='ReLU', switch_norm=False, name='sep_conv')
    ...
    for i in range(stack_num):
        ...
        if switch_norm:
            X = SwitchhNormalization(name='{}_{}_depthwise_BN'.format(name, i))(X)
        ...
        if switch_norm:
            X = SwitchhNormalization(name='{}_{}_pointwise_BN'.format(name, i))(X)

        X = activation_func(name='{}_{}_pointwise_activation'.format(name, i))(X)
    
    return X

def ASPP_conv(X, channel, activation='ReLU', Switch_norm=True, name='aspp'):
    '''
    Atrous Spatial Pyramid Pooling (ASPP).
    
    ASPP_conv(X, channel, activation='ReLU', Switch_norm=True, name='aspp')
    ...
    * dilation rates are fixed to `[6, 9, 12]`.
    ...
    if switch_norm:
        b4 = SwitchNormalization(name='{}_conv_b4_BN'.format(name))(b4)
    ...
    if switch_norm:
        b0 = SwitchNormalization(name='{}_conv_b0_BN'.format(name))(b0)
    ...
    b_r6 = Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, activation='ReLU', 
                        dilation_rate=6, switch_norm=True, name='{}_sepconv_r6'.format(name))
    b_r9 = Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, activation='ReLU', 
                        dilation_rate=9, switch_norm=True, name='{}_sepconv_r9'.format(name))
    b_r12 = Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, activation='ReLU', 
                        dilation_rate=12, switch_norm=True, name='{}_sepconv_r12'.format(name))
